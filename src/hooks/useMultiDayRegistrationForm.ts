...
export const useMultiDayRegistrationForm = ({ setView, setActiveDate }: UseMultiDayRegistrationFormProps) => {
    ...
    useEffect(() => {
        const fetchRegisteredDates = async () => {
            ...
            // FIX: Explicitly type the result of the map to string[] before creating the Set.
            const datesWithData = new Set<string>(registrations.filter(r => r.count > 0).map(r => r.date));
